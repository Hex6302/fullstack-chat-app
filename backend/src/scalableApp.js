...
import { performanceMiddleware, errorTrackingMiddleware } from "./lib/monitoring.js";
import { createScalableSocketServer, setupSocketHandlers } from "./lib/scalableSocket.js";
...
app.get("/api/health", async (req, res) => {
  try {
    const { getMetrics, getHealthStatus } = await import('./lib/monitoring.js');
...
const startServer = async () => {
  try {
    // Enhanced Socket.IO setup for scalability
    const io = await createScalableSocketServer(server);
    const socketHandlers = setupSocketHandlers(io);
    
    // Store io globally for shutdown
    global.io = io;
